...
export default function GlitchEffect({
  ...
  cyanColor = "var(--accent)",
  className = "",
}: GlitchEffectProps) {
  const reduced = useReducedMotion();
  const uid = `glitch-${text.replace(/\s+/g, "").slice(0, 6).toLowerCase()}`;

  // Base rule always applies. Glitch layers (pseudo-elements, keyframes,
  // hover trigger) are omitted entirely under reduced motion — clean text,
  // never glitches even on hover.
  const baseRule = `
    .${uid} {
      position: relative;
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: baseRule + glitchRules }} />
